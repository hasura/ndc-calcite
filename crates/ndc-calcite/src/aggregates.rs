...
#[tracing::instrument]
pub(crate) fn numeric_aggregates(underlying_type: &str) -> BTreeMap<String, AggregateFunctionDefinition> {
    let aggregate_functions: BTreeMap<String, AggregateFunctionDefinition> = ["sum", "max", "avg", "min"]
        .iter()
        .map(|function| (
            function.to_string(),
