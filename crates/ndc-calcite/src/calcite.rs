...
#[tracing::instrument]
fn create_calcite_connection<'a>(calcite_query: &JObject<'a>, env: &'a mut JNIEnv<'a>) -> Result<JValueOwned<'a>, InitializationError> {
...
#[tracing::instrument]
pub fn calcite_query(calcite_ref: GlobalRef, query: &str, query_metadata: &models::Query) -> Result<Vec<Row>, QueryError> {
    ...
    match result.unwrap() {
        Object(obj) => {
            let j_string = JString::from(obj);
            let json_string: String = env.get_string(&j_string).unwrap().into();
            let json_array: Result<Vec<Row>, serde_json::Error> = serde_json::from_str(&json_string);
