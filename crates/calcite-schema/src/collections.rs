...
use ndc_models::{
    CollectionInfo, CollectionName, FieldName, ForeignKeyConstraint, ObjectField, ObjectType,
    ObjectTypeName, ScalarType, ScalarTypeName, Type, TypeName, UniquenessConstraint,
};
...
pub fn collections(
    ...
    for (table, table_metadata) in data_models {
        ...
        if !scalar_types.contains_key(&tbl_name) {
            object_types.insert(
                ObjectTypeName::new(table.to_string().into()),
                ObjectType {
            ...
            collection_infos.push(CollectionInfo {
                name: CollectionName::new(table.clone().into()),
                description: Some(format!("A collection of {table}")),
                collection_type: ObjectTypeName::new(TypeName::from(table.to_string())),
            ...
            return Err(ErrorResponse::new(
                ...
                format!("Table names cannot be same as a scalar type name: {table}",),
                Value::Null,
            ));
        }
    }
...
fn build_fields(
    column_metadata: &HashMap<FieldName, ColumnMetadata>,
) -> BTreeMap<FieldName, ObjectField> {
    column_metadata
        .iter()
        .map(|(column_name, column_metadata)| {
            let scalar_type = TypeName::from(column_metadata.scalar_type.clone());
            let nullable = column_metadata.nullable;
            let final_type: Type = if nullable {
                Nullable {
                    underlying_type: Box::new(Named { name: scalar_type }),
                }
            } else {
...
fn build_uniqueness_constraints(
    ...
    unique_constraints.insert(
        ...
        UniquenessConstraint {
            unique_columns: table_metadata
                .primary_keys
                .clone()
                .unwrap()
                .iter()
                .map(|s| FieldName::new(s.clone().parse().unwrap()))
                .collect(),
        },
    );
    unique_constraints
...
fn build_foreign_keys(
    table_metadata: &TableMetadata,
    models_data_map: &HashMap<CollectionName, TableMetadata>,
) -> BTreeMap<String, ForeignKeyConstraint> {
                ...
                && foreign_key.fk_table_name == table_metadata.name
            {
                ...
                match foreign_key_constraints.get_mut(&primary_table_name) {
                    ...
                    None => {
                        ...
                        new_constraint.column_mapping.insert(
                            FieldName::from(foreign_key.fk_column_name),
                            FieldName::from(foreign_key.pk_column_name),
                        );
                    ...
                    Some(existing_constraint) => {
                        existing_constraint.column_mapping.insert(
                            FieldName::from(foreign_key.fk_column_name),
                            FieldName::from(foreign_key.pk_column_name),
                        );
                    }
