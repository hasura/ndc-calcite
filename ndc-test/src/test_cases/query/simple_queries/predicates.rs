...
use rand::Rng;
use ndc_models::ComparisonOperatorDefinition;
        ...
        if let Some(field_scalar_type) = schema.scalar_types.get(field_type_name.as_str()) {
            for (operator_name, operator) in &field_scalar_type.comparison_operators {
                match operator {
                    ...
                    ComparisonOperatorDefinition::GreaterThan => {
                        let value = values.choose(rng).ok_or(Error::ExpectedNonEmptyRows)?;
                    ...
                    ComparisonOperatorDefinition::LessThan => {
                        let value = values.choose(rng).ok_or(Error::ExpectedNonEmptyRows)?;
                    ...
                    ComparisonOperatorDefinition::GreaterThanOrEqualsTo => {
                        let value = values.choose(rng).ok_or(Error::ExpectedNonEmptyRows)?;
                    ...
                    ComparisonOperatorDefinition::LessThanOrEqualsTo => {
                        let value = values.choose(rng).ok_or(Error::ExpectedNonEmptyRows)?;
