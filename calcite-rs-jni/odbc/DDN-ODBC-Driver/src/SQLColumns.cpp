...
SQLRETURN SQLColumns_Impl(
    ...
    std::string catalogName = szCatalogName && cbCatalogName != 0
                                  ? std::string(szCatalogName,
                                                cbCatalogName == SQL_NTS ? strlen(szCatalogName) : cbCatalogName)
                                  : "";
    std::string schemaName = szSchemaName && cbSchemaName != 0
                                 ? std::string(szSchemaName,
                                               cbSchemaName == SQL_NTS ? strlen(szSchemaName) : cbSchemaName)
                                 : "";
    ...
    std::string columnName = szColumnName && cbColumnName != 0
                                 ? std::string(szColumnName,
                                               cbColumnName == SQL_NTS ? strlen(szColumnName) : cbColumnName)
                                 : "";
...
SQLRETURN SQL_API SQLColumns_W(
    ...
    std::wstring wsCatalogName = (szCatalogName && cbCatalogName != 0)
                                     ? std::wstring(szCatalogName,
                                                    cbCatalogName == SQL_NTS ? wcslen(szCatalogName) : cbCatalogName)
                                     : L"";
    std::wstring wsSchemaName = (szSchemaName && cbSchemaName != 0)
                                    ? std::wstring(szSchemaName,
                                                   cbSchemaName == SQL_NTS ? wcslen(szSchemaName) : cbSchemaName)
                                    : L"";
    std::wstring wsTableName = (szTableName && cbTableName != 0)
                                   ? std::wstring(szTableName,
                                                  cbTableName == SQL_NTS ? wcslen(szTableName) : cbTableName)
                                   : L"";
    std::wstring wsColumnName = (szColumnName && cbColumnName != 0)
                                    ? std::wstring(szColumnName,
                                                   cbColumnName == SQL_NTS ? wcslen(szColumnName) : cbColumnName)
                                    : L"";
